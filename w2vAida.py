...
class Word2Vec(utils.SaveLoad):
    ...
    """
    def __init__(self, sentences=None, size=200, alpha=0.025, window=5, min_count=5,
        sample=0, seed=1, workers=8, min_alpha=0.0001, sg=1, hs=0, negative=5, cbow_mean=0):
        """
        ...
        self.vocab = defaultdict(list)  # mapping from a word (string) to a Vocab object
    ...
    def save_word2vec_format_new(self, fname, fvocab=None, binary=False):
        """
        Store the input-hidden weight matrix in the same format used by the original
        C word2vec-tool, for compatibility.
        """
        ...
        with utils.smart_open(fname, 'wb') as fout:
            ...
            for word, vocab in sorted(iteritems(self.vocab), key=lambda item: -item[1].count):
                ...
                if binary:
                    array=word.split(":")
                    print(array[0] , row)
                    fout.write(utils.to_utf8(array[0]) + b" " + row.tostring())
                    #print "next"
                    #print array
                else:
                    fout.write(utils.to_utf8("%s %s\n" % (word, ' '.join("%f" % val for val in row))))
    ...
    def most_similar_new(self, positive=[], negative=[], topn=1):
        #print('most_similar new')
        self.init_sims()
        ...
        if not wtwo or not wthree:
            return [("z",0.0)]
        else:
            for x, y in [(x,y) for x in wtwo for y in wthree]:
        ...
        if not wone:
            ...
            for x, y in [(x,y) for x in wone for y in k]:
                l1 = Word2Vec.similarity(self, x , y)
                l2 = x,y
                l3 = tuple([l1]+[l2])
                o.append(l3)
        m = max(o,key=itemgetter(0))[1]
        j3 = ''.join(elem for elem in m[0])

        f1 = Word2Vec.most_similar(self,positive=[j3, j2], negative=[j1],topn=1)

        r = []
        for x in wone:
            r.append(Word2Vec.most_similar(self,positive=[x, j2], negative=[j1],topn=1))

        f2 = max(r,key=itemgetter(0))
        #print(f1, ' ' , f2)
        f3 = f2 > f1
        if f3 is True:
            return f1
    ...
    def similarity_new(self,one=[],two=[]):
        self.init_sims()
    ...
    def n_similarity_new(self,one=[],two=[]):
        ...
        if not wthree or not wfour:
            return (0.0)
        else:
            for x, y in [(x,y) for x in wthree for y in wfour]:
                o0 = Word2Vec.similarity(self, x , y)
                o1 = x,y
                o2 = tuple([o0]+[o1])
                o.append(o2)
            m = max(o,key=itemgetter(0))[1]
    ...
    def similar_word(self, word):
        ...
        for w in self.vocab:
            #w1=re.split(':',w)
            simi = self.similarity(w, word)
            res.append((w, simi))
        #answer = sorted(res, cmp=lambda x, y:-cmp(x[1], y[1]))[:50]
        return res

    def save_word_vectors(self, filename):
...
if __name__ == "__main__":
    ...
    model = Word2Vec(LineSentence(infile), size=200, min_count=3, workers=8)
    model = Word2Vec(Text8Corpus(infile), size=200, min_count=3, workers=8)
